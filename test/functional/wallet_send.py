...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Use fee_rate (bit/vB) instead of feeRate", w0.send, {w1.getnewaddress(): 1}, 6, "conservative", 1, {"feeRate": 0.01})
        ...
        # Test setting explicit fee rate just below the minimum of 0.010 bit/vB.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' if fee_rate of 0.009 is passed")
        msg = "Fee rate (0.009 bit/vB) is lower than the minimum fee rate setting (0.010 bit/vB)"
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, fee_rate=0.009, expect_error=(-4, msg))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_fee_rate=0.009, expect_error=(-4, msg))
