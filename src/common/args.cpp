...
const char * const BITCOIN_CONF_FILENAME = "bitfuc.conf";
...
{
    // Windows: %LOCALAPPDATA%\Bitfuc
    // macOS: ~/Library/Application Support/Bitfuc
    // Unix-like: ~/.bitfuc
    // Never fall back to a Bitcoin datadir.
#ifdef WIN32
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Bitfuc";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    return pathRet / "Library/Application Support/Bitfuc";
#else
    return pathRet / ".bitfuc";
#endif
#endif
}
